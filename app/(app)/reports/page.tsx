...
import { getKanbanData, getReportData } from "@/lib/data/documents";
...
export default async function ReportsPage({
  ...
  const threshold = activeWorkspace?.confidence_threshold ?? 0.9;
  const currency = activeWorkspace?.currency ?? "PHP";

  const kanbanDocs = activeWorkspace ? await getKanbanData(activeWorkspace.id, threshold) : [];
  const monthDocs = activeView === "report" && activeWorkspace
    ? await getReportData(activeWorkspace.id, selectedMonth)
    : [];

  // Category totals for monthly report
  ...
  // Kanban counts
  const resolvedDocs = kanbanDocs.filter((d) => d.isResolved);
  const discussionDocs = kanbanDocs.filter((d) => !d.isResolved && d.hasComments);
  const attentionDocs = kanbanDocs.filter((d) => !d.isResolved && !d.hasComments && d.needsAttention);
    ...
    <>
      ...
      <div>
        ...
        <div className="screen-h no-print">
          <div>
            <h1>Reports</h1>
            <p>{activeWorkspace ? activeWorkspace.name : "Select a workspace"} — problem tracking & monthly report</p>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {activeView === "report" && <PdfButton month={selectedMonth} />}
          </div>
        </div>

        {/* View tabs */}
        <div className="no-print" style={{ display: "flex", gap: 4, marginBottom: 24, borderBottom: "1px solid var(--ink-200)", paddingBottom: 0 }}>
          <a
            href="?view=kanban"
            style={{
              padding: "8px 16px",
              ...
              fontWeight: 600,
              textDecoration: "none",
              borderBottom: activeView === "kanban" ? "2px solid var(--blue-600)" : "2px solid transparent",
              color: activeView === "kanban" ? "var(--blue-600)" : "var(--ink-500)",
              marginBottom: -1,
            }}
          >
            Kanban
            {attentionDocs.length > 0 && (
              <span style={{ marginLeft: 6, fontSize: 11, fontWeight: 700, background: "oklch(0.93 0.08 60)", color: "oklch(0.45 0.14 60)", padding: "1px 6px", borderRadius: 8 }}>
          ...
          <a
            ...
            style={{
              ...
              fontWeight: 600,
              textDecoration: "none",
              borderBottom: activeView === "report" ? "2px solid var(--blue-600)" : "2px solid transparent",
              color: activeView === "report" ? "var(--blue-600)" : "var(--ink-500)",
              marginBottom: -1,
            }}
          >
            Monthly report
        ...
        {activeView === "report" && (
          <>
            ...
            <div style={{ display: "none" }} className="print-only">
              ...
              <p style={{ margin: "0 0 20px", color: "#666" }}>Expense Report — {displayMonth}</p>
            </div>
            ...
            <div className="no-print" style={{ marginBottom: 24 }}>
              <form method="GET">
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <select name="month" defaultValue={selectedMonth} className="ds-select" style={{ width: "auto" }}>
                    {monthOptions.map((o) => (
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
                <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700, letterSpacing: "-0.02em" }}>{displayMonth}</h2>
                <span style={{ fontSize: 13, color: "var(--ink-500)" }}>{activeWorkspace?.name ?? ""}</span>
              </div>
            </div>
            ...
            {monthDocs.length === 0 ? (
              ...
              <>
                <div className="card" style={{ marginBottom: 20 }}>
                  <div className="card-h"><div className="card-title">Summary by category</div></div>
                  <table className="tbl">
                    <thead>
                      <tr>
                        <th>Category</th>
                ...
                <div className="card">
                  <div className="card-h"><div className="card-title">Document detail</div></div>
                  <table className="tbl">
                    <thead>
                      <tr>
                        <th>Date</th>
                      ...
                      {monthDocs.map((doc) => {
                        ...
                        return (
                          <tr key={doc.id}>
                            ...
                            <td style={{ color: "var(--ink-600)", fontSize: 13 }}>{String(ext.suggested_category ?? "—")}</td>
                            <td className="mono" style={{ fontSize: 12, color: "var(--ink-500)" }}>{String(ext.reference_number ?? "—")}</td>
                            <td className="mono" style={{ textAlign: "right", fontWeight: 600 }}>
